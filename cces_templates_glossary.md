...
- **permission_duty** (OPTIONAL): Boolean to indicate that the permission has a duty (defaults to `false`)
- **permission_duty_target** (OPTIONAL): The target of the permission duty (ex: https://example.com/assets_2) (**ONLY** if `permission_duty` = `true`)
- **permission_duty_action** (OPTIONAL): The action of the permission duty (ex: http://www.w3.org/ns/odrl/2/compensate) (**ONLY** if `permission_duty` = `true`)
- **permission_duty_action_refinement** (OPTIONAL): Boolean to indicate that the permission duty action has a refinement (**ONLY** if `permission_duty_action` = `true`)
